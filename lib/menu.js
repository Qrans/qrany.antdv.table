...
import Vue from "vue";
import { copy, is_null } from "./utils";
  ...
  computed: {
    menu() {
      return [
        ...
        { key: "format", disabled: !this.content.cells.length, title: "复制列" },
      ];
    },
  },
...
const get_dom = (dom, tag) => {
  if (dom.tagName === tag) return dom;
  if (dom.parentElement) return get_dom(dom.parentElement, tag);
  return null;
};
