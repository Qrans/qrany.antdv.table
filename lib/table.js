...
export const QTable = Vue.extend({
  ...
  props: {
    ...
    selectProps: { type: [undefined, Function], validater: (v) => !v || typeof v === "function" },
  ...
  methods: {
    ...
    table$(h) {
      const select = this.select ? { type: this.select, fixed: true, selectedRowKeys: this.selectedRowKeys, onChange: this.onSelection } : null;
