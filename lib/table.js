...
export const QTable = Vue.extend({
  ...
  props: {
    ...
    selectProps: { type: Function, default: undefined },
  ...
  methods: {
    onAutoSelect(v) {
      ...
      this.ctx.select(this.data.map(map), this.data);
    },
    onSorts(v) {
      this.sorts = v;
    },
    ...
    diffSelect(rows) {
      const old_rows = this.table.selections;
      if (!old_rows.length && !this.table.auto) return;
      const map = typeof this.rowKey === "function" ? this.rowKey : (x) => x[this.rowKey];
      ...
      const old_json = old_rows.reduce((c, x) => ({ ...c, [map(x)]: JSON.stringify(x) }), {});
      const new_json = rows.reduce((c, x) => ({ ...c, [map(x)]: JSON.stringify(x) }), {});
      const new_rows = [];
      for (const x of old_rows) {
        const k = map(x);
    ...
    table$(h) {
      const select = this.select ? { type: this.select, fixed: true, selectedRowKeys: this.selectedRowKeys, onChange: this.onSelection, getCheckboxProps: this.selectProps } : null;
      const table = [
        this.tips$(h),
        h(
          Table,
