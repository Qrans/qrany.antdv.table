...
export const QTable = Vue.extend({
  ...
  methods: {
    ...
    tips$(h) {
      return h(QTips, {
        props: { settings: this.settings, select: this.table.selectedRowKeys.length, options: this.sorts },
        on: { "update:options": this.onSorts, reload: this.reload },
      });
    },
        ...
        h(
          Table,
          {
            props: {
              ...
              rowSelection: { type: this.select, fixed: true, selectedRowKeys: this.table.selectedRowKeys, onChange: this.ctx.select },
              scroll: { x: true },
