...
import Vue from "vue";
import "ant-design-vue/es/alert/style/index";
...
import "ant-design-vue/es/popover/style/index";
...
export const QTips = Vue.extend({
  ...
  methods: {
    ...
    select$(h) {
      const s = [
        h("span", "已选择"),
        h("a", { attrs: { href: "javascript:;" } }, `${this.count}`),
        h("span", "项"),
        h("a", { attrs: { href: "javascript:;" }, on: { click: this.onClear } }, "清空"),
      ];
    ...
    message$(h) {
      const row = [h(Col, [h(Space, [h(Tooltip, { props: { title: "刷新" } }, [h(Icon, { props: { type: "reload" }, on: { click: this.onReload } })]), this.popover$(h)])])];
      if (this.has) row.unshift(h(Col, [h(Space, this.select$(h))]));
      return h(Row, { props: { type: "flex", justify: this.has ? "space-between" : "end" } }, row);
    },
  },
